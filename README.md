...
├── checkExpired.js         # Отдельный скрипт для проверки истекших блюд
├── render.yaml             # Конфигурация Render.com (опционально)
├── package.json            # Зависимости проекта
├── setup_users.sql         # SQL для создания таблицы users
├── setup_user_settings.sql # SQL для создания таблицы user_settings
└── README.md               # Этот файл
```
...
- Отображает все активные блюда с датой истечения (в формате UTC)
...
Бот автоматически отправляет уведомления (все времена в UTC):
...
- **При истечении срока**: уведомление о необходимости списания (проверяется каждую минуту через `checkExpired.js`)

**Важно**: Все времена отображаются и обрабатываются в часовом поясе UTC
...
1. Проверьте, что переменные окружения установлены в Render Dashboard
2. Проверьте логи в Render Dashboard → Logs
3. Убедитесь, что BOT_TOKEN валидный (проверьте через `@BotFather`)

### Уведомления не приходят

1. Проверьте логи в Render Dashboard → Logs, ищите `[SCHEDULER]` и `[CHECK_EXPIRED]`
2. Убедитесь, что `chat_id` сохраняется при создании блюда
3. Проверьте, что scheduler запускается (должны быть логи каждую минуту)
4. Можно вручную запустить проверку: `npm run check-expired`
